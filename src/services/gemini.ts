...
const OPENAI_DEEP_MODEL = process.env.OPENAI_DEEP_MODEL || "gpt-5";
...
async function callOpenAI(messages: OpenAIMessage[], options?: { temperature?: number; responseFormat?: any; model?: string }): Promise<string> {
  if (!apiKey) {
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      model: options?.model || OPENAI_FAST_MODEL,
      messages,
...
`;
  ...
  const content = await callOpenAI(
    ...
    { temperature: 0.2, model: OPENAI_DEEP_MODEL }
  );
