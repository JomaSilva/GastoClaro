...
export async function generateBatchSignals(marketData: any[]): Promise<any[]> {
  const assetsWithNews = marketData.filter((data) => data.news && data.news.length > 0);
  const assetsWithoutNews = marketData.filter((data) => !data.news || data.news.length === 0);
...
`;
    ...
    try {
      ...
      console.error("Error generating/parsing batch signals", error);
    }
  }

  const defaultResults = assetsWithoutNews.map((data) => ({
    symbol: data.symbol,
    signal: "NEUTRO",
    strength: 50,
